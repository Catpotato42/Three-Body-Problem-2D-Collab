...
5. Input desired simulation length. This is the length of time the computer will simulate the orbit of the planets for, not the real time the simulation lasts. This is in units of 10,000 seconds.
6. Input relative speed. This is the frames per 1,000 seconds of simulation we output. So, at 100k seconds (a value of 10 input into Sim Length) a relative speed of 1 gets us (100,000 s calculated / 1,000 s per frame output) * 1 frame = 100 frames output.
7. Press Create.
8. Then, enter initial values (x-pos, y-pos, x-vel, y-vel, mass) for each planet.
9. Position variables are in pixels from the top left of the screen (positive down and right), which has its width and height printed at the top left when entering initial values. This is not dynamic so resizing the window after the first screen is not recommended. pixels are translated 1 pixel per 1e6 (1 million) meters. 
10. Velocity is in m/s, + is right/down, - is left/up. a couple thousand should give movement that looks reasonable.
11. Mass is in 1e24 kg. The gravitational constant is the same as the real world, at 6.6743e-11 m^3/kgs^2.
12. Press Start Simulation when ready! Don't worry if it says not responding or shows a black screen for more than 30 seconds, just don't click anything for a while. Longer simulations where fpss*simulation length > 1000 may take a couple minutes to load.

### Notes
Simulation speed is much higher than the real world (1,000 seconds per each real-world frame at a Relative Speed of 1), as orbital periods are measured in the spans of days to years in the real world - some planets in our solar system take thousands of years to orbit the sun.
